import type { RobloxAsset } from "./types"

// Sample data structure with migrated and mock data
export const assets: RobloxAsset[] = [
  // Migrated KOL Characters from kolDatabase.ts
  {
    id: "k1",
    name: "TechTrix",
    description: "Tech-savvy gaming influencer with a vibrant personality",
    assetType: "kol_character",
    robloxAssetId: "12345671",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["gaming", "tech", "influencer"],
    characterType: "Streamer",
    appearance: {
      gender: "Female",
      style: ["Streetwear", "Tech-wear", "Urban"],
      hairStyle: "Neon Purple Bob Cut",
      hairColor: "Purple",
      height: "Medium",
      features: ["RGB Glasses", "Holographic Accessories", "Tech Tattoos"],
    },
    personality: ["Energetic", "Tech-Savvy", "Trendy"],
    defaultAnimations: ["Victory Dance", "Gaming Pose", "Energetic Wave"],
    suitableFor: {
      brands: ["Gaming Peripherals", "Energy Drinks", "Tech Gadgets"],
      products: ["Gaming Chairs", "Headphones", "Custom PC Builds"],
      gameTypes: ["Esports Arenas", "Gaming Cafes", "Tech Conventions"],
    },
    marketingCapabilities: ["Brand Integration", "Live Game Reviews", "Tech Unboxing"],
  },

  // Migrated Clothing from customizationDatabase.ts
  {
    id: "c1",
    name: "Nike Hoodie",
    description: "Comfortable Nike branded hoodie for casual wear",
    assetType: "clothing",
    robloxAssetId: "12345672",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["nike", "hoodie", "casual"],
    image: "/customization/outfits/Nike-hoodie.png",
    previewImage: "/customization/outfits/Nike-hoodie-preview.png",
    compatibility: ["Gamer", "Athlete", "Lifestyle", "Artist", "Fashion"],
    brands: ["Nike"],
    size: ["S", "M", "L", "XL"],
  },

  // Migrated Minigame from gameplayDatabase.ts
  {
    id: "mg1",
    name: "Lucky Spin Wheel",
    description: "Interactive spinning wheel with customizable segments and brand placements",
    assetType: "minigame",
    robloxAssetId: "12345673",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["game", "interactive", "wheel"],
    difficulty: "Easy",
    maxPlayers: 1,
    gameplayDuration: "45 seconds",
    customizableElements: [
      {
        id: "wheel-bg",
        name: "Wheel Background",
        type: "image",
        description: "Background image behind the wheel",
      },
    ],
    marketingCapabilities: ["Brand Awareness", "Product Showcase", "Lead Generation"],
  },

  // Mock Hat Asset
  {
    id: "h1",
    name: "Supreme Baseball Cap",
    description: "Limited edition Supreme branded baseball cap",
    assetType: "hat",
    robloxAssetId: "12345674",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["supreme", "cap", "streetwear"],
    image: "/customization/hats/supreme-cap.png",
    compatibility: ["Lifestyle", "Fashion"],
    brands: ["Supreme"],
  },

  // Mock Animation Asset
  {
    id: "a1",
    name: "Victory Dance",
    description: "Celebratory dance animation for victory moments",
    assetType: "animation",
    robloxAssetId: "12345675",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["dance", "celebration", "victory"],
    duration: "3.5 seconds",
    category: "emote",
    previewUrl: "/animations/victory-dance-preview.mp4",
    compatibility: ["All"],
  },

  // Mock Image Asset
  {
    id: "img1",
    name: "Brand Logo Collection",
    description: "High-resolution brand logos for in-game displays",
    assetType: "image",
    robloxAssetId: "12345676",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["logos", "branding", "marketing"],
    url: "/images/brand-logos.png",
    dimensions: {
      width: 1920,
      height: 1080,
    },
    fileFormat: "png",
    fileSize: 2048,
  },

  // Mock Audio Asset
  {
    id: "aud1",
    name: "Victory Fanfare",
    description: "Triumphant music for victory moments",
    assetType: "audio",
    robloxAssetId: "12345677",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["music", "victory", "fanfare"],
    url: "/audio/victory-fanfare.mp3",
    duration: "5.5 seconds",
    fileFormat: "mp3",
    fileSize: 1024,
  },

  // Mock Video Asset
  {
    id: "vid1",
    name: "Brand Introduction",
    description: "Video introduction for brand partnership",
    assetType: "video",
    robloxAssetId: "12345678",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["brand", "intro", "marketing"],
    url: "/videos/brand-intro.mp4",
    duration: "30 seconds",
    dimensions: {
      width: 1920,
      height: 1080,
    },
    fileFormat: "mp4",
    fileSize: 15360,
  },

  // Additional migrated items from customizationDatabase.ts
  {
    id: "s1",
    name: "Dr. Martens Boots",
    description: "Classic Dr. Martens boots for a bold style statement",
    assetType: "shoes",
    robloxAssetId: "12345679",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["boots", "dr-martens", "fashion"],
    image: "/customization/outfits/DrMartenBoots.png",
    compatibility: ["Artist", "Fashion"],
    brands: ["Dr. Martens"],
    size: ["UK6", "UK7", "UK8", "UK9", "UK10"],
  },

  // Additional migrated items from gameplayDatabase.ts
  {
    id: "mg2",
    name: "Brand Hoops",
    description: "Basketball shooting game with customizable court and billboard advertisements",
    assetType: "minigame",
    robloxAssetId: "12345680",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["sports", "basketball", "interactive"],
    difficulty: "Medium",
    maxPlayers: 4,
    gameplayDuration: "2 minutes",
    customizableElements: [
      {
        id: "court-floor",
        name: "Court Floor",
        type: "image",
        description: "Basketball court floor branding",
      },
    ],
    marketingCapabilities: ["Brand Integration", "Sports Marketing", "Event Activation"],
  },

  // Additional KOL Characters from kolDatabase.ts
  {
    id: "k2",
    name: "SportStar",
    description: "Professional athlete influencer focused on sports and fitness",
    assetType: "kol_character",
    robloxAssetId: "12345681",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["sports", "fitness", "athlete"],
    characterType: "Athlete",
    appearance: {
      gender: "Male",
      style: ["Athletic", "Sporty", "Modern"],
      hairStyle: "Fade Cut",
      hairColor: "Black",
      height: "Tall",
      features: ["Athletic Build", "Sports Accessories", "Team Jersey"],
    },
    personality: ["Competitive", "Motivational", "Team Player"],
    defaultAnimations: ["Sports Celebration", "Training Routine", "Victory Pose"],
    suitableFor: {
      brands: ["Sports Apparel", "Fitness Equipment", "Health Supplements"],
      products: ["Athletic Shoes", "Sports Gear", "Energy Bars"],
      gameTypes: ["Sports Games", "Training Simulators", "Competition Venues"],
    },
    marketingCapabilities: ["Brand Endorsements", "Fitness Challenges", "Sports Events"],
  },
  {
    id: "k3",
    name: "ArtisticSoul",
    description: "Creative digital artist and design influencer",
    assetType: "kol_character",
    robloxAssetId: "12345682",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["art", "design", "creative"],
    characterType: "Artist",
    appearance: {
      gender: "Neutral",
      style: ["Artistic", "Bohemian", "Creative"],
      hairStyle: "Colorful Undercut",
      hairColor: "Rainbow",
      height: "Medium",
      features: ["Paint Splatter Clothes", "Digital Tablet", "Creative Accessories"],
    },
    personality: ["Creative", "Expressive", "Inspiring"],
    defaultAnimations: ["Drawing Pose", "Creative Expression", "Artistic Flourish"],
    suitableFor: {
      brands: ["Art Supplies", "Digital Tools", "Creative Software"],
      products: ["Drawing Tablets", "Art Materials", "Creative Tools"],
      gameTypes: ["Art Studios", "Creative Spaces", "Design Workshops"],
    },
    marketingCapabilities: ["Art Tutorials", "Creative Collaborations", "Digital Art Showcases"],
  },
  {
    id: "k4",
    name: "StyleIcon",
    description: "Fashion and lifestyle influencer with a keen eye for trends",
    assetType: "kol_character",
    robloxAssetId: "12345683",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["fashion", "lifestyle", "luxury"],
    characterType: "Fashion Influencer",
    appearance: {
      gender: "Female",
      style: ["High Fashion", "Trendy", "Luxury"],
      hairStyle: "Long Wavy",
      hairColor: "Blonde",
      height: "Tall",
      features: ["Designer Accessories", "Premium Outfits", "Runway Walk"],
    },
    personality: ["Sophisticated", "Trendsetter", "Charismatic"],
    defaultAnimations: ["Catwalk", "Photo Pose", "Elegant Wave"],
    suitableFor: {
      brands: ["Fashion Brands", "Luxury Items", "Beauty Products"],
      products: ["Designer Clothes", "Accessories", "Cosmetics"],
      gameTypes: ["Fashion Shows", "Shopping Centers", "Beauty Studios"],
    },
    marketingCapabilities: ["Fashion Showcases", "Style Tips", "Brand Collaborations"],
  },

  // Additional Clothing from customizationDatabase.ts
  {
    id: "c2",
    name: "Adidas Sport Shirt",
    description: "Performance sport shirt from Adidas",
    assetType: "clothing",
    robloxAssetId: "12345684",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["adidas", "sports", "athletic"],
    image: "/customization/outfits/White-shirt-Adidas.png",
    compatibility: ["Athlete", "Lifestyle"],
    brands: ["Adidas"],
    size: ["S", "M", "L", "XL"],
  },
  {
    id: "c3",
    name: "Supreme T-Shirt",
    description: "Streetwear essential Supreme t-shirt",
    assetType: "clothing",
    robloxAssetId: "12345685",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["supreme", "streetwear", "casual"],
    image: "/customization/outfits/White-Shirt-Supreme.png",
    compatibility: ["Lifestyle", "Artist"],
    brands: ["Supreme"],
    size: ["S", "M", "L", "XL"],
  },
  {
    id: "c4",
    name: "Gucci Jacket",
    description: "Luxury Gucci designer jacket",
    assetType: "clothing",
    robloxAssetId: "12345686",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["gucci", "luxury", "jacket"],
    image: "/customization/outfits/Color-jacket-Gucci.png",
    compatibility: ["Fashion", "Lifestyle"],
    brands: ["Gucci"],
    size: ["S", "M", "L", "XL"],
  },

  // Additional Shoes from customizationDatabase.ts
  {
    id: "s2",
    name: "Nike Air Max",
    description: "Classic Nike Air Max sneakers",
    assetType: "shoes",
    robloxAssetId: "12345687",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["nike", "sneakers", "athletic"],
    image: "/customization/outfits/MixColor-Nike-AirMax.png",
    compatibility: ["Athlete", "Lifestyle"],
    brands: ["Nike"],
    size: ["US7", "US8", "US9", "US10", "US11"],
  },
  {
    id: "s3",
    name: "Adidas Superstar",
    description: "Iconic Adidas Superstar sneakers",
    assetType: "shoes",
    robloxAssetId: "12345688",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["adidas", "sneakers", "casual"],
    image: "/customization/outfits/Red-shoes-Adidas-Superstar.png",
    compatibility: ["Lifestyle", "Fashion"],
    brands: ["Adidas"],
    size: ["US7", "US8", "US9", "US10", "US11"],
  },

  // Additional Items from customizationDatabase.ts
  {
    id: "i1",
    name: "Coca-Cola Can",
    description: "Interactive Coca-Cola can prop",
    assetType: "item",
    robloxAssetId: "12345689",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["drink", "prop", "beverage"],
    image: "/customization/items/CocaCola.png",
    brands: ["Coca-Cola"],
    itemType: "prop",
  },
  {
    id: "i2",
    name: "Pepsi Bottle",
    description: "Interactive Pepsi bottle prop",
    assetType: "item",
    robloxAssetId: "12345690",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["drink", "prop", "beverage"],
    image: "/customization/items/Pepsi.png",
    brands: ["Pepsi"],
    itemType: "prop",
  },
  {
    id: "i3",
    name: "Red Bull Energy",
    description: "Interactive Red Bull can prop",
    assetType: "item",
    robloxAssetId: "12345691",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["drink", "prop", "energy"],
    image: "/customization/items/Redbull.png",
    brands: ["Red Bull"],
    itemType: "prop",
  },
  {
    id: "i4",
    name: "Monster Energy",
    description: "Interactive Monster Energy can prop",
    assetType: "item",
    robloxAssetId: "12345692",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["drink", "prop", "energy"],
    image: "/customization/items/MonsterEnergy.png",
    brands: ["Monster"],
    itemType: "prop",
  },

  // Additional Animations from customizationDatabase.ts
  {
    id: "a2",
    name: "Dance Move",
    description: "Hip-hop style dance animation",
    assetType: "animation",
    robloxAssetId: "12345693",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["dance", "hip-hop", "emote"],
    duration: "4.0 seconds",
    category: "emote",
    previewUrl: "/customization/animations/hiphop.png",
    compatibility: ["Lifestyle", "Artist"],
  },
  {
    id: "a3",
    name: "Victory Pose",
    description: "Celebratory victory pose animation",
    assetType: "animation",
    robloxAssetId: "12345694",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["victory", "celebration", "emote"],
    duration: "3.0 seconds",
    category: "emote",
    previewUrl: "/customization/animations/Victory.png",
    compatibility: ["Athlete", "Gamer"],
  },
  {
    id: "a4",
    name: "Drink Animation",
    description: "Generic drinking animation",
    assetType: "animation",
    robloxAssetId: "12345695",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["drink", "action", "emote"],
    duration: "2.5 seconds",
    category: "action",
    previewUrl: "/customization/animations/Drinking.png",
  },

  // Additional Minigames from gameplayDatabase.ts
  {
    id: "mg3",
    name: "Brand Puzzle",
    description: "Interactive puzzle game featuring brand imagery",
    assetType: "minigame",
    robloxAssetId: "12345696",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["puzzle", "interactive", "educational"],
    difficulty: "Medium",
    maxPlayers: 1,
    gameplayDuration: "5 minutes",
    customizableElements: [
      {
        id: "puzzle-image",
        name: "Puzzle Image",
        type: "image",
        description: "Main puzzle image",
      },
    ],
    marketingCapabilities: ["Product Education", "Brand Storytelling", "Visual Showcase"],
  },
  {
    id: "mg4",
    name: "Brand Racer",
    description: "Racing game with customizable tracks and billboards",
    assetType: "minigame",
    robloxAssetId: "12345697",
    createdAt: "2024-03-01",
    updatedAt: "2024-03-20",
    tags: ["racing", "sports", "competitive"],
    difficulty: "Hard",
    maxPlayers: 8,
    gameplayDuration: "4 minutes",
    customizableElements: [
      {
        id: "track-billboards",
        name: "Track Billboards",
        type: "image",
        description: "Trackside advertising billboards",
      },
      {
        id: "vehicle-wrap",
        name: "Vehicle Wrap",
        type: "3d-model",
        description: "Custom vehicle branding",
      },
    ],
    marketingCapabilities: ["Brand Visibility", "Product Placement", "Competitive Gaming"],
  },
]

